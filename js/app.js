...
(function () {
  ...
  const BLOCK_META = {
    ...
    tireoide: { color: '#0891b2', soft: 'rgba(8,145,178,0.22)',   icon: '🦋' },
    reumato:  { color: '#9333ea', soft: 'rgba(147,51,234,0.22)',  icon: '🔴' },
    cardio:   { color: '#e11d48', soft: 'rgba(225,29,72,0.22)',   icon: '❤️' },
    hepato:   { color: '#d97706', soft: 'rgba(217,119,6,0.22)',   icon: '🫀' },
    itu:      { color: '#0284c7', soft: 'rgba(2,132,199,0.22)',   icon: '🦠' },
    revisao:  { color: '#0f766e', soft: 'rgba(15,118,110,0.22)',  icon: '🎯' },
  };
  ...
  function buildSidebar() {
    ...
    const blockOrder = ['entrada', 'tireoide', 'reumato', 'cardio', 'hepato', 'itu', 'revisao'];
  ...
  function applyTheme(theme) {
    ...
    localStorage.setItem('go_theme', theme);
    const icon = document.getElementById('theme-icon');
    if (icon) icon.textContent = theme === 'dark' ? '☀️' : '🌙';
  }

  applyTheme(localStorage.getItem('go_theme') || 'light');
  ...
  function buildProgressModal() {
    ...
    ['entrada','tireoide','reumato','cardio','hepato','itu','revisao'].forEach(blockId => {
